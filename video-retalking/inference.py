...
def main():
    ...
    if args.exp_img is not None and (".png" in args.exp_img or ".jpg" in args.exp_img):
        ...
        lm_exp = kp_extractor.extract_keypoint(
            [exp_pil], "temp/" + base_name + "_temp.txt"
        )[0]
        if np.mean(lm_exp) == -1:
        ...
        im_exp_tensor = (
            torch.tensor(np.array(im_exp) / 255.0, dtype=torch.float32)
            .permute(2, 0, 1)
            .to(device)
            .unsqueeze(0)
        )
    ...
    gen = datagen(
        imgs_enhanced.copy(), mel_chunks, full_frames, None, (oy1, oy2, ox1, ox2)
    )
    ...
    # step7 face_enhance
    video_fp = args.outfile
    out_fp = os.path.splitext(video_fp)[0] + "_enhanced.mp4"
    face_enhance(
        restorer=restorer,
        video_fp=args.outfile,
        out_fp=out_fp,
...
def face_enhance(
    restorer, video_fp, out_fp, fps=30, frame_width=1920, frame_height=1080
):
    from basicsr.utils import imwrite
    import shutil
    import glob

    enhance_dir = "./temp/enhance"
    if os.path.exists(enhance_dir):
        shutil.rmtree(enhance_dir)
    os.makedirs(enhance_dir)
    frame_dir = os.path.join(enhance_dir, "images")
    os.makedirs(os.path.join(frame_dir))

    basedir = os.getcwd()
    os.chdir(frame_dir)
    command = f"ffmpeg -y -i {video_fp} -r {fps} -f image2 -s {frame_width}x{frame_height} %5d.png"
    os.system(command=command)
    os.chdir(basedir)

    img_list = sorted(glob.glob(f"{frame_dir}/*.png"))
    extension = "png"
    # ------------------------ restore ------------------------
    for img_path in tqdm(img_list, desc="[Step 7] Face Enhance:"):
        # read image
        ...
        for idx, (cropped_face, restored_face) in enumerate(
            ...
            save_restore_path = os.path.join(
                enhance_dir, "restored_imgs", f"{basename}.{extension}"
            )
            imwrite(restored_img, save_restore_path)

    out_fp = os.path.abspath(out_fp)
    os.chdir(os.path.join(enhance_dir, "restored_imgs"))
    command = f"ffmpeg -y -f image2 -r {fps} -i %05d.png -strict -2 -r {fps} {out_fp}"
    os.system(command)
    os.chdir(basedir)

    print(f"Results are in the [{out_fp}] folder.")
